...
pub trait SourceDbQuerier: Send + Sync + fmt::Debug {
    fn query_source_db(
        ...
        let mut argfile = NamedTempFile::with_prefix("pyrefly_build_query_")
            .with_context(|| "Failed to create temporary argfile for querying source DB".to_owned())?;
        ...
        if !result.status.success() {
            let stdout = String::from_utf8(result.stdout).unwrap_or_else(|_| {
                "<Failed to parse stdout from source DB query>".to_owned()
            });
