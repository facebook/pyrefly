...
def check_project(
    ...
    # Try migrating from pyright first for apples-to-apples comparison.
    # If that fails (no pyright config), fall back to default init.
    result = run(
    ...
    if result.returncode != 0:
        logging.info("  No pyright config found, falling back to default init")
