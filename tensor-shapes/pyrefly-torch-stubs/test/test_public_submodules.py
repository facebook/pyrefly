...
def inference_identity[Shape: IntTuple](tensor: Tensor[Shape]) -> Tensor[Shape]:
    return tensor
...
def test_torch_submodules_are_attributes() -> None:
    ...
    assert_shape(inference_identity(torch.ones((2, 3))).shape, (2, 3))


if TYPE_CHECKING:
    _ = Adafactor, Adam, Muon, default_eval_fn
    assert_type(torch.OutOfMemoryError, type[torch.OutOfMemoryError])
