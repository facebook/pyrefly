...
def test_arithmetic_function_shapes() -> None:
    ...
    # TODO: BUG: Broadcast both operands for these pointwise functions.
    assert_shape(torch.hypot(left, right).shape, (2, 1), runtime=(2, 3))
    assert_shape(torch.lerp(left, right, 0.5).shape, (2, 1), runtime=(2, 3))
    assert_shape(torch.fmod(left, right).shape, (2, 1), runtime=(2, 3))
    assert_shape(torch.remainder(left, right).shape, (2, 1), runtime=(2, 3))
    assert_shape(torch.copysign(left, right).shape, (2, 1), runtime=(2, 3))
    assert_shape(torch.nextafter(left, right).shape, (2, 1), runtime=(2, 3))
...
def test_arithmetic_method_shapes() -> None:
    ...
    # TODO: BUG: Broadcast both operands for these pointwise methods.
    assert_shape(left.hypot(right).shape, (2, 1), runtime=(2, 3))
    assert_shape(left.lerp(right, 0.5).shape, (2, 1), runtime=(2, 3))
    assert_shape(left.fmod(right).shape, (2, 1), runtime=(2, 3))
    assert_shape(left.remainder(right).shape, (2, 1), runtime=(2, 3))
    assert_shape(left.copysign(right).shape, (2, 1), runtime=(2, 3))
    assert_shape(left.nextafter(right).shape, (2, 1), runtime=(2, 3))
...
if TYPE_CHECKING:
    ...
    def check_symbolic_arithmetic[N: IntVar, M: IntVar](
        ...
        assert_type(torch.hypot(left, right), Tensor[[N, 1]])
        assert_type(torch.lerp(left, right, 0.5), Tensor[[N, 1]])
        assert_type(torch.fmod(left, right), Tensor[[N, 1]])
        assert_type(torch.remainder(left, right), Tensor[[N, 1]])
        assert_type(torch.copysign(left, right), Tensor[[N, 1]])
        assert_type(torch.nextafter(left, right), Tensor[[N, 1]])
        assert_type(left.hypot(right), Tensor[[N, 1]])
        assert_type(left.lerp(right, 0.5), Tensor[[N, 1]])
        assert_type(left.fmod(right), Tensor[[N, 1]])
        assert_type(left.remainder(right), Tensor[[N, 1]])
        assert_type(left.copysign(right), Tensor[[N, 1]])
        assert_type(left.nextafter(right), Tensor[[N, 1]])
