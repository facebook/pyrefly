...
def test_masked_select_has_data_dependent_length() -> None:
    ...
    assert_shape(torch.masked_select(tensor, mask).shape, (int,), runtime=(3,))
    assert_shape(tensor.masked_select(mask).shape, (int,), runtime=(3,))

    empty_mask = torch.zeros((2, 3), dtype=torch.bool)
    assert_shape(tensor.masked_select(empty_mask).shape, (int,), runtime=(0,))
...
if TYPE_CHECKING:
    ...
    def check_symbolic[N: IntVar, M: IntVar](
        ...
        assert_type(torch.masked_select(tensor, mask), Tensor[[int]])
        assert_type(tensor.masked_select(mask), Tensor[[int]])
