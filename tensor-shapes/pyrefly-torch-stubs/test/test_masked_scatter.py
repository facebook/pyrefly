...
def test_masked_scatter_can_expand_the_input() -> None:
    ...
    # TODO: BUG: Out-of-place masked_scatter should return the broadcasted shape.
    assert_shape(
        torch.masked_scatter(tensor, mask, source).shape,
        (2, 3),
        runtime=(1, 2, 3),
    )
    assert_shape(
        tensor.masked_scatter(mask, source).shape,
        (2, 3),
        runtime=(1, 2, 3),
    )
...
if TYPE_CHECKING:
    ...
    def check_gradual_mask(
        ...
        assert_type(tensor.masked_scatter(mask, source), Tensor[[2, 3]])
