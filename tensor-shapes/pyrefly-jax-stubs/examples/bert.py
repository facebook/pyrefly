...
def test_bert_model() -> None:
    ...
    out = bert(x, segment)
    assert_shape(out.shape, (4, 128, 256))
...
def test_bert_default_hidden() -> None:
    ...
    out = bert(x, segment)
    assert_shape(out.shape, (4, 128, 768))
...
def test_bert_lm() -> None:
    ...
    assert_shape(nsp_out.shape, (4, 2))
    assert_type(nsp_out, Array[[4, 2]])
    assert_shape(mlm_out.shape, (4, 128, 30522))
...
def test_bert_pytree_transformations() -> None:
    ...
    assert_shape(vmap_out.shape, (3, 2, 16, 64))
