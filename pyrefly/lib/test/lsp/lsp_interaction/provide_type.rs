...
fn test_provide_type() {
    do_test(6, 9, "builtins.type[bar.Bar]");
}

#[test]
fn test_provide_type_basic() {
    do_test(7, 5, "typing.Literal[3]");
}

#[test]
fn test_provide_type_nested_class() {
    do_test(9, 13, "builtins.type[bar.Bar.Baz]");
}

#[test]
fn test_provide_type_function_fully_qualified() {
    do_test(15, 9, "def bar.Bar.Baz.f(self: bar.Bar.Baz) -> None");
}

#[test]
fn test_provide_type_variable() {
    do_test(19, 4, "T@bar.f_generic1");
}

#[test]
fn test_provide_generic_signature() {
    do_test(21, 4, "(T@bar.f_generic2) -> T@bar.f_generic2");
}

#[test]
fn test_provide_generic_signature_with_declaration() {
    do_test(24, 0, "[T: builtins.int = builtins.bool](t: T) -> T");
}

#[test]
fn test_self() {
    do_test(11, 14, "typing.Self@bar.Bar.Baz");
}

#[test]
fn test_builtin_method_type() {
    do_test(30, 8, "def builtins.int.bit_length(self: builtins.int) -> builtins.int");
}

#[test]
fn test_builtin_function_type() {
    do_test(31, 0, "def builtins.any(iterable: typing.Iterable[builtins.object], /) -> builtins.bool");
}

#[test]
fn test_function_type_with_constraints() {
    do_test(33, 8, "def bar.f_constraints[T: (builtins.bool, builtins.str) = builtins.bool](t: T) -> T");
}

#[test]
fn test_bound_method() {
    do_test(16, 10, "def bar.Bar.Baz.f() -> None");
}

#[test]
fn test_super() {
    do_test(37, 10, "builtins.super[builtins.object, bar.S]");
}
...
fn test_new_type() {
    ...
    do_test(39, 1, "builtins.type[bar.N]");
}

#[test]
fn test_overload() {
    do_test(50, 0, "Overload[def bar.overloaded_func(x: None) -> None, def bar.overloaded_func(x: builtins.int) -> None]");
}

#[test]
fn test_generic_mixed_scopes() {
    do_test(56, 14, "def bar.A.f1.B.f2[F2](self: typing.Self@bar.A.f1.B, x: F1@bar.A.f1, y: F2, a: T@bar.A, b: T2@bar.A.f1.B) -> None");
}

#[test]
fn test_tuple() {
    do_test(58, 0, "builtins.tuple[typing.Literal[1], typing.Literal[2]]");
}

#[test]
fn test_tuple_item() {
    do_test(58, 11, "typing.Literal[2]");
}

#[test]
#[ignore = "the type alias resolves to Unknown"]
fn test_type_alias() {
    do_test(62, 6, "def bar.alias() -> bar.TA");
}

#[test]
#[ignore = "the signature is returned, not the result"]
fn test_pos() {
    do_test(68, 0, "typing.Literal[False]");
}