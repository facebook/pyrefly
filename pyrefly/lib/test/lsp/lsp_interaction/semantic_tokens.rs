...
#[test]
fn semantic_tokens_format_specifier_escapes() {
    let interaction = LspInteraction::new();
    interaction
        .initialize(InitializeSettings::default())
        .unwrap();

    let uri = Url::parse("untitled:Untitled-4").unwrap();
    // Test that escape sequences like \t, \n, \x41, \u1234 before a format specifier
    // do not cause the token highlight offset to shift.
    let text = "import logging\nlogger = logging.getLogger()\nlogger.info(\"\\t\\n\\x41\\u0042 %s, item \\t %d\", \"world\", 42)\n";
    interaction.client.did_open_uri(&uri, "python", text);
...
#[test]
fn semantic_tokens_format_specifier_named_width_precision() {
    let interaction = LspInteraction::new();
    interaction
        .initialize(InitializeSettings::default())
        .unwrap();

    let uri = Url::parse("untitled:Untitled-5").unwrap();
    let text = r#"
msg = "User %(user)s has %(count)05d items with price %10.2f" % {"user": "Alice", "count": 3, "price": 9.99}
"#;
    interaction.client.did_open_uri(&uri, "python", text);
        ...
        .expect_response_with(move |response| match response {
            Some(SemanticTokensResult::Tokens(tokens)) => {
                let specifiers: Vec<_> = decode_semantic_tokens(&tokens, &legend, text)
                    ...
                    .map(|(_, text)| text)
                    .collect();
                specifiers == ["%(user)s", "%(count)05d", "%10.2f"]
            }
            _ => false,
        })
        .unwrap();
