...
# `Want(1, upper=2)` may use `upper` both as a positional-only name and in `**kwargs`.
def posonly_collision(upper: int = 0, /, lower: int = 0, **kwargs: int) -> None: ...
ok_posonly: Want = posonly_collision
    "#,
);

// An empty unpack does not fill an optional positional parameter before a later keyword.
testcase!(
    test_non_constant_param_mapping_empty_unpack,
    r#"
from typing import Protocol, Unpack

class Want(Protocol):
    def __call__(self, *args: Unpack[tuple[()]], b: int) -> None: ...
...
// Each of these rejects at least one call form permitted by `Want`.
testcase!(
...
# `Want(1, upper=2)` passes `upper` both positionally and by name; `**kwargs` cannot absorb it.
def duplicate_binding(upper: int = 0, lower: int = 0, **kwargs: int) -> None: ...
e5: Want = duplicate_binding  # E: is not assignable to `Want`
    "#,
);
