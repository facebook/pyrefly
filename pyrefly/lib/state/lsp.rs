...
impl<'a> Transaction<'a> {
    ...
    fn class_attribute_type_at_definition(
        ...
        let class_ty = answers.get_type_at(bindings.key_to_idx(&class_key))?;
    ...
    fn type_from_resolution(
        ...
        match kind {
            ...
            ResolutionKind::Key(key) => {
                ...
                let mut ty = if matches!(context, IdentifierContext::Expr(ExprContext::Store)) {
                    self.class_attribute_type_at_definition(handle, identifier)
                        .or_else(|| answers.get_type_at(bindings.key_to_idx(&key)))?
                } else {
                    answers.get_type_at(bindings.key_to_idx(&key))?
                };
