                    ...
                    if let Some(mut idx) = self.bindings().key_to_idx_hashed_opt(Hashed::new(&key))
                    {
                        ...
                        let partial_assign = loop {
                            ...
                            match self.bindings().get(idx) {
                                ...
                                Binding::NameAssign(assign) if assign.def_idx.is_some() => {
                                    break Some(assign);
                                }
                                _ => break None,
                            }
                        };
                        if let Some(assign) = partial_assign {
                            let reveal_partial = match &assign.first_use {
                                FirstUse::Undetermined | FirstUse::DoesNotPin => true,
                                FirstUse::UsedBy(first_use_idx) => {
                                    let first_use_key = self.bindings().idx_to_key(*first_use_idx);
                            ...
                            let override_ty = if reveal_partial {
                                match &*assign.expr {
                                    Expr::List(x) if x.elts.is_empty() => {
                                        ...
                                        Some(self.heap.mk_class_type(self.stdlib.list(elem)))
                                    }
                                    Expr::Dict(x) if x.items.is_empty() => {
                                        ...
                                        Some(
                                            self.heap
                                                .mk_class_type(self.stdlib.dict(key_ty, value_ty)),
                                        )
                                    }
                                    _ => None,
                                }
