...
impl PolarsFunction {
    fn from_id(id: &FuncDefId) -> Self {
        ...
        let module = module.as_str();
        if matches!(module, "narwhals.functions" | "narwhals.translate") {
            return match name {
                "col" => Self::Col,
                "concat" => Self::Concat,
                "from_native" => Self::FromNative,
                "len_" => Self::Len,
                "lit" => Self::Lit,
                "when" => Self::When,
                _ => Self::Unmodeled,
...
impl<'ctx, 'answer, Ans: LookupAnswer> AnswersSolver<'ctx, 'answer, Ans> {
    ...
    pub(crate) fn infer_polars_call_specialization(
        ...
        match PolarsFunction::from_callee(callee) {
            ...
            // Narwhals wraps a native frame without reshaping it, so a schema already tracked for
            // the backend frame carries over to the Narwhals one.
            Some(PolarsFunction::FromNative) => {
                let [native] = &arguments.args[..] else {
                    return None;
                };
                let Type::DataFrame(schema) = self.expr_infer(native, &self.error_swallower())
                else {
                    return None;
                };
                return Some(PolarsCallSpecialization::DataFrame {
                    columns: schema.columns.clone(),
                    kind: DataFrameKind::Narwhals,
