    ...
    pub fn solve_expectation(
        ...
        match binding {
            ...
            BindingExpect::ForwardRefUnion {
                ...
                // Check if one side is a forward reference string literal and the other
                // side is a plain type. At runtime, `type.__or__` cannot handle string
                // literals, so expressions like `int | "str"` will raise a TypeError.
                ...
                if let Some(l) = self.untype_opt(lhs.clone(), left.range(), errors)
                    && let Some(r) = self.untype_opt(rhs.clone(), right.range(), errors)
                {
                    let is_plain_type =
                        |t: &Type| matches!(t, Type::ClassType(cls) if cls.targs().is_empty());
                    if (*left_is_forward_ref && is_plain_type(&r))
                        || (*right_is_forward_ref && is_plain_type(&l))
                    {
                        self.error(
                            errors,
                            *range,
