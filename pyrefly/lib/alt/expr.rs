...
impl<'a, Ans: LookupAnswer> AnswersSolver<'a, Ans> {
    ...
    pub fn expr_infer_type_info_with_hint(
        ...
        let res = match x {
            Expr::Name(x) => {
                if let Some(info) = self.lambda_param_type_info(x) {
                    info
                } else {
    ...
    fn expr_infer_type_no_trace(
        ...
        match x {
            ...
            Expr::Lambda(lambda) => self.lambda_infer(lambda, hint, errors),
            Expr::Tuple(x) => self.tuple_infer(x, hint, errors),
            Expr::List(x) => {
                let elt_hint = hint.and_then(|hint| self.decompose_list(hint));
    ...
    fn boolop(
        ...
        for (i, value) in values.iter().enumerate() {
            ...
            let hint_ref = hint.clone().or_else(|| {
                if t_acc.is_never() {
                    None
                } else {
                    soft_hint = Some(self.soft_hint_from_type(t_acc.clone()));
                    Some(soft_hint.as_ref().unwrap().as_ref())
                }
            });
