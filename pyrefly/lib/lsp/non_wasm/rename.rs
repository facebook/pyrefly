...
pub(crate) fn append_comment_and_string_occurrences(
    ...
    let mut merged: HashMap<ModuleInfo, Vec<TextRange>> = HashMap::new();
    for (module_info, ranges) in references.drain(..) {
        merged.insert(module_info, ranges);
    }
    ...
    for candidate in tx.handles() {
        ...
        if let Some(root) = workspace_root {
            if !candidate.path().as_path().starts_with(root) {
                continue;
            }
        }
        ...
        merged
            .entry(module_info)
            .or_default()
            .extend(comment_ranges);
    }

    *references = merged.into_iter().collect();
}
