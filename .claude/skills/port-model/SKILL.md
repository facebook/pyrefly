...
The `torch_shapes` package bridges pyrefly's type system and Python runtime.
...
runtime (e.g., for runtime shape validation), import `torch_shapes` directly
(not under `TYPE_CHECKING`). Use old-style `torch_shapes.TypeVar` instead of
