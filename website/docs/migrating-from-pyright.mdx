...
Pyright's Windows, Linux, Darwin (macOS), and All platform settings are preserved during migration. Pyrefly's `all` platform keeps platform-dependent expressions unconstrained rather than selecting one specific platform.
