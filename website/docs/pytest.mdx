...
def test_connection(database_url):
    reveal_type(database_url)  # revealed type: str
```
...
def test_lookup(user_id):
    reveal_type(user_id)  # revealed type: int
```
...
In the editor, Pyrefly can offer:

- `Add pytest fixture type annotation`, which updates the fixture to
  `def enabled() -> bool:`.
- `Add pytest fixture parameter type annotation`, which updates the test to
  `def test_feature(enabled: bool):`.
- Batch variants that annotate all eligible fixtures or fixture parameters in
  the file.

The parameter inlay hints are controlled by
`python.analysis.inlayHints.pytestParameters`, which defaults to `false`.
