...
A bare `*Bs` splat checks identically — `Elements` is the explicit inverse of
`tuple[Unpack[...]]` for shape type variables, and it only matters when the
annotation itself is evaluated at runtime, since unpacking a bare `TypeVar`
raises `TypeError`.
