...
[jaxtyping](https://github.com/patrick-kidger/jaxtyping) annotations. Support is
disabled by default. Enable it for a project or
[`sub-config`](./configuration.mdx#sub-config) with:
...
is either a Pyrefly shaped-array class or a generic class with one gradual
`IntTuple`-bounded shape argument. The currently supported shape syntax is:
