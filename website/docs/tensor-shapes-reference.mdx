...
[jaxtyping](https://github.com/patrick-kidger/jaxtyping) annotations. A function
opts in by declaring its dimensions with `@static_jaxtyping`; see the
[overview](./tensor-shapes.mdx#jaxtyping-runtime-type-checking). An annotation in
a function without a declaration keeps its ordinary `Annotated` meaning.
...
Within a function, repeated names represent the same dimension and participate
in inference across parameters and returns. Supported annotations are translated
internally to generics and display back in jaxtyping syntax:
...
Full static support would require implementing those semantics and extending
the compatibility tests across the remaining jaxtyping grammar and container
forms. The cross-method/class limitation also requires an upstream annotation
model for shared dimensions; see the
