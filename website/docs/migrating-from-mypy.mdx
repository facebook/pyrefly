...
We recommend reviewing the resulting config. While there is some overlap between mypy's and Pyrefly's config options, not every option or error code has an exact equivalent.
...
The `ignore_missing_imports` config option is migrated to `ignore-missing-imports`. For example:
...
```toml
ignore-missing-imports = ["some.*.module"]
```

Mypy's `follow_imports = "skip"` is migrated to `replace-imports-with-any`. Unlike `ignore-missing-imports`, this always replaces matching modules with `typing.Any`, even when Pyrefly can resolve them.
...
Mypy's global `platform`, `check_untyped_defs`, and `strict` settings are recognized in both `mypy.ini` and `pyproject.toml`'s `[tool.mypy]` section during migration. `check_untyped_defs` and `strict` enable `check-unannotated-defs` while keeping `infer-return-types = "never"`, matching mypy's treatment of unannotated return values.
...
While not every error code has an equivalent error kind, we make an effort to ensure that Pyrefly suppresses the same errors that mypy does. Error codes without a Pyrefly equivalent are omitted from the generated config and reported as migration warnings, so audit the generated `[errors]` table against the original `enable_error_code` and `disable_error_code` settings.

Mapped codes may lead to overly broad error suppressions, and you may want to consider removing some error kinds from the disable list.
