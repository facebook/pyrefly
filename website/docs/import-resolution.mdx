...
Pass `--full` to also print every other resolved configuration setting, including error
severity overrides, path-scoped `sub-config` overrides, and any config keys Pyrefly did not
recognize. This is useful when a setting does not appear to take effect and you need to see
the value Pyrefly actually resolved.
