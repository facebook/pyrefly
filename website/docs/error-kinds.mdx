...
Note that bad overrides caused by inconsistent parameter names are separately reported as [bad-override-param-name](#bad-override-param-name), and bad overrides caused by mutable attribute type changes are separately reported as [bad-override-mutable-attribute](#bad-override-mutable-attribute). The mutable-attribute kind is reserved for overrides that fail only because of mutability (narrowing or widening); an attribute override whose type is incompatible with the parent's in both directions is reported as plain `bad-override`.

## bad-override-mutable-attribute

Arises when a subclass overrides a mutable (read-write) attribute of a parent class with a type that differs only in a way mutability forbids (narrowing or widening).
...
If the child's type is incompatible with the parent's in both directions, the override is reported as [bad-override](#bad-override) instead, so that disabling this error kind cannot mask the worse incompatibility.
...
## incompatible-overload-argument
...
Deprecated: this error code has been renamed to [incompatible-overload-argument](#incompatible-overload-argument).
The old name is still accepted in suppression comments and configuration for backwards compatibility.
...
This error is raised when code can never execute because it comes after a statement that
always exits the current flow, such as `return`, `raise`, `break`, or `continue`. One
diagnostic is reported per block, covering everything from the first dead statement to the
end of the block.

```python
def example():
    return 1
    print("dead")  # This code is unreachable [unreachable]
    print("also dead, but covered by the same diagnostic")

def loop_example():
    while True:
        break
        return 1  # This code is unreachable [unreachable]
```

A dead region consisting only of `yield`s is not reported. That is how a generator
is written: the `yield` is what makes the function a generator. A dead
`yield` alongside any other dead statement is ordinary dead code and is still reported.

```python
def empty_generator():
    return
    yield  # Not reported: this is what makes the function a generator

def mixed():
    return
    yield  # Not reported: this might be used to make the function a generator
    print("hello!")  # This code is unreachable [unreachable]
```

It is also raised for the body of an `if` or `while` whose condition is statically known,
and for the branches that a chosen branch makes dead:
...
A condition that consults the runtime environment — `sys.version_info`, `sys.platform`,
`os.name`, or `TYPE_CHECKING` — is **not** reported. Such a branch is dead only under the
configuration being checked and is live under another, so flagging it would be a false
positive on code that deliberately supports several environments:

```python
import sys

if sys.version_info < (3, 10):
    print("not reported")   # live on older interpreters

if sys.platform == "win32":
    print("not reported")   # live on Windows
```

A dead `if` branch is skipped entirely, so nothing inside it is analysed. A `while` body is
still bound and type-checked as ordinary code, so real problems inside it are reported.

Unreachable regions are grayed out in the editor. Blocks disabled by the environment (e.g.
`sys.platform == "linux"`) are grayed out, but do not receive an error.
