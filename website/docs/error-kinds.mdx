...
This error occurs when attempting to call an abstract method directly. Abstract methods are defined using `@abc.abstractmethod` and must be implemented in subclasses before they can be called.
...
obj = AbstractClass()
obj.abstract_method()  # Cannot call abstract method - must be implemented in a subclass [abstract-method-call]
```

This error also applies to abstract static methods and abstract class methods:
...
AbstractClass.abstract_static_method()  # Cannot call abstract method [abstract-method-call]
AbstractClass.abstract_class_method()   # Cannot call abstract method [abstract-method-call]
```

To fix this error, implement the abstract method in a concrete subclass:
