...
Assigning to a method on a class object or instance is ambiguous because Python's
type system cannot distinguish the bound and unbound callable types that the
replacement will produce. This check is enabled by the `strict` preset.

```python
class Service:
  def run(self) -> None: ...

def replacement(self: Service) -> None: ...

Service.run = replacement  # method-assign
```
