...
## invalid-return-in-finally

A `return`, `break`, or `continue` statement that directly exits a `finally` block. Starting in Python 3.14 ([PEP 765](https://peps.python.org/pep-0765/)), this is disallowed because it can silently mask exceptions.
...
The following are allowed because they don't exit the finally block:
...
def f():
    try:
        ...
        def inner():
            return 42  # OK: return is in nested function
