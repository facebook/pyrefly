...
Narwhals has no public frame constructor, and `from_native`, `from_dict`, and
the rest of the Narwhals IO surface are not modeled, so those calls produce an
opaque frame. A tracked schema starts only from a
[schema declaration](#schema-declaration) on a parameter or return type.
...
def process(df: Annotated[nw.DataFrame, Sales]) -> None:
