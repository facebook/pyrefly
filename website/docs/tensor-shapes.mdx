...
[Variadic dimensions](./tensor-shapes-reference.mdx#variadic-dimensions)
...
class Linear[N: IntVar, M: IntVar]:
    ...
    def forward[Xs: IntTuple](self, inp: Tensor[[*Xs, N]]) -> Tensor[[*Xs, M]]:
        ...
...
Pyrefly has experimental static support for a subset of jaxtyping annotations as
an alternative front-end to our native syntax. A function opts in to shape
checking by declaring its dimensions with `@static_jaxtyping`:
...
def scale(x: Float[Tensor, "batch channels"]) -> Float[Tensor, "batch channels"]:
    return x * 2
```

The declaration lists the dimension names the function's shape strings may use,
separated by spaces, with a leading `*` marking a variadic shape. Using a name
the declaration does not list is an error.

Outside the scope of an enclosing declaration, a jaxtyping annotation keeps its
ordinary `Annotated` meaning and the array shape stays gradual. Shape checking
can therefore be adopted one definition at a time.

A class declaration makes the class generic in its dimensions, allowing
attributes and methods to share them. These relationships are static and are not
enforced by a runtime checker such as `beartype`.

Supported annotations are translated internally to use generics. See the
[compatibility reference](./tensor-shapes-reference.mdx#jaxtyping-compatibility)
for class examples, supported syntax, and remaining gaps.
