...
Without the decorator a jaxtyping annotation keeps its ordinary `Annotated`
meaning and the array shape stays gradual, so shape checking can be adopted one
function at a time, and a file being migrated away from jaxtyping is not
enforced while it is in progress.
...
for the supported syntax and remaining gaps.
