...
Which Pyrefly binary the extension launches. `from-environment` looks for a Pyrefly installed in your active Python environment, so your editor and your command line agree on a version; if there isn't one, the bundled binary is used. `bundled` always uses the binary shipped with the extension, whose version is tied to the extension version.

#### `pyrefly.lspPath`

**Type:** string &nbsp; **Default:** `""`

If your platform is not supported, you can build pyrefly from source and specify the binary path with this setting. When set, it overrides `pyrefly.pyreflyExecutable`.
