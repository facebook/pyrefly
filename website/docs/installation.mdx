...
Pyrefly is available on [PyPI](https://pypi.org/project/pyrefly/) with a new minor release intended every ~2 months.
...
For tab completion setup, see [Shell completion](../shell-completion).

## Configure

Pyrefly with no configuration [automatically finds nearby legacy type checking configurations](../configuration/#unconfigured-projects). If we cannot find one, or migration fails, we use the **basic preset**, which shows only high-confidence errors that are very likely to indicate real bugs or crashes (like syntax errors and missing imports) and silences other diagnostics.

To unlock full type checking and pin your configuration, run `pyrefly init`. This writes a `pyrefly.toml` (or updates `pyproject.toml`) and explicitly migrates any existing mypy/pyright settings. All [configuration options are documented here](../configuration). Common knobs to tune include:
...
Run Pyrefly from your project directory to type-check the whole project:

```sh
pyrefly check
```

You can also check specific files or directories:

```sh
pyrefly check src/ tests/ path/to/file.py
```

For feedback while you edit, install a [Pyrefly IDE extension](../IDE). It provides diagnostics, hover information, inlay hints, and navigation directly in the editor. For more details, run `pyrefly help`.
