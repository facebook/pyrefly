...
that can apply to a given file when any `matches` glob pattern matches.
...
SubConfigs whose `matches` do not match the given file, then takes the first non-null
...
matches = ["sub/project/tests/file.py", "sub/project/tests/**/*.py"]
...
matches = ["sub/project/**", "sub/project/tests/**"]
