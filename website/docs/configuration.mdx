...
installed third-party packages with `typing.Any` if the package provides neither
stubs nor a `py.typed` marker. Stubs and `py.typed` packages are analyzed
normally, and first-party modules are not affected.

- Type: list of regex
- Default: `[]`
- Flag equivalent: `--replace-untyped-imports-with-any`
- Equivalent config: `follow_untyped_imports = false` in mypy; use `["*"]` to
  apply it to every module
