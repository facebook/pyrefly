...
Treat module-level `ALL_CAPS` names as final after their first assignment. When
enabled, assigning to the same constant name again emits a `bad-assignment`
error, similar to writing an explicit `Final` annotation.
