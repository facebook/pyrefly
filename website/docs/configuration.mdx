...
:::info
Want all type error squiggles to show up in your editor by default? Try setting `python.pyrefly.displayTypeErrors` to `"force-on"` in your editor settings, create a `pyrefly.toml` file in your project root, or add a `[tool.pyrefly]` section to your `pyproject.toml` (can be empty).
:::
...
below this threshold are hidden from output and do not affect the exit code.
...
- Notes:
    - `min-severity` is a **project-level setting** and cannot be overridden in
      [`sub-config`](#sub-configs) sections.
    - This setting controls CLI display only. To change the severity of individual
...
- Type: string (`"off"`, `"basic"`, `"legacy"`, `"default"`, `"strict"`)
...
A minimal-checking preset for unconfigured projects and LSP users. Only the highest-confidence diagnostics — crashes and clearly broken code — are active; every other error kind is silenced.
...
- Disables: [`bad-override-mutable-attribute`](./error-kinds.mdx#bad-override-mutable-attribute), [`bad-override-param-name`](./error-kinds.mdx#bad-override-param-name)
