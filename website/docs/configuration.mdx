...
Controls whether Pyrefly checks every `match` statement for exhaustiveness.

By default, Pyrefly checks only closed subject types, such as enums, literal
unions, `None`, `bool`, and non-subclassable classes. When this option is
enabled, Pyrefly also reports `non-exhaustive-match` for open-domain subjects
when its narrowing logic can prove that execution may fall through the match.

```toml
check-all-matches = true
```

This option does not affect the severity of the diagnostic. Configure
[`non-exhaustive-match`](./error-kinds.mdx#non-exhaustive-match) in the
`[errors]` table to change its severity or disable it.

Use `typing.assert_never` in a fallback arm when exhaustiveness should be
required for one specific match without enabling this project-wide policy.

- Type: `bool`
- Default: `false`
- Flag equivalent: `--check-all-matches`
...
4. Starting from the project root, look for `pyvenv.cfg` in a `.venv`, `venv`, or `env`
   ...
   `pyvenv.cfg`, Pyrefly probes the standard interpreter paths: `bin/python3`, `bin/python`,
   `python3`, and `python` on macOS and Linux, or `Scripts/python.exe` and `python.exe` on
   Windows. Environments in other directories or with only a version-specific executable
   such as `python3.12` must be activated or set explicitly with
...
# should Pyrefly check every match statement for exhaustiveness?
check-all-matches = false
