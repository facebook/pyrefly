...
- `root` is the absolute path to the root of the repository. Source paths in
  `srcs` are relative to this root.
...
| `relative_to` | no | Directory, relative to `root`, that this target's `srcs` resolve against instead of `root` itself. A build system sets this when it materializes generated sources outside the source tree, such as the output directory of a Buck genrule. Every `src` of a target that sets this counts as generated. |
