...
Pyrefly reads `stdlib/VERSIONS` from its bundled typeshed to hide standard library modules
that do not exist on the configured [`python-version`](#python-version). That check does
**not** apply to a custom typeshed: a directory supplied here is searched as-is, even though
a typeshed checkout ships its own `stdlib/VERSIONS`. So with `typeshed-path` set,
`import chunk` resolves on Python 3.13 despite the module being removed in that release,
whereas the bundled typeshed reports it as missing.
