...
The language server finds a configuration separately for each source file because one
session can serve multiple projects. The language server also runs the search for each
imported file. The per-file search can produce different IDE and project-mode CLI results.
See [IDE configuration selection](./IDE.mdx#configuration-selection) for details and ways
to configure external source trees.
...
- Notes:
    - The language server selects a configuration for each imported file separately from
      resolving the import through `search-path`. See
      [IDE configuration selection](./IDE.mdx#configuration-selection) for details.
...
Pyrefly reads `stdlib/VERSIONS` to hide standard library modules that do not exist on the
configured [`python-version`](#python-version). A typeshed supplied here is filtered by **its
own** `stdlib/VERSIONS`, not the bundled one, since it replaces Pyrefly's standard library
rather than adding to it. A module that file declares removed will not fall back to Pyrefly's
bundled stub, though it can still resolve from an installed package on the
[`site-package-path`](#site-package-path).

A typeshed with no `stdlib/VERSIONS` — such as one produced by materializing Pyrefly's bundled
stubs — is not version-filtered at all, and Pyrefly logs a warning saying so.
...
can be found, its type information will still be replaced with `typing.Any`. Go-to-definition
can still open the module source when source is available.
...
If a target's `config` or the top-level `default_config` names an entry that
`configs` does not define, Pyrefly rejects the whole response and reports the
error instead of type checking. An undefined name supplies no settings, which is
indistinguishable from a config that sets nothing, so accepting it would mean
silently checking your repository under the wrong settings.
...
Build-system provided configs take precedence over matching sub-configs because
they describe the most specific build-system scope.

`default_config` names the entry that supplies any setting a target's own config
leaves unset. It also applies to files that belong to no target at all, but
are still part of the current config, such as a file opened in your editor that
the build system does not know about, so a build system can be used to dynamically
configure settings.

The full resolution chain follows this order, taking the first that sets a value:
target config → `default_config` → sub-config → top-level config →
[preset](#preset) defaults. Settings that are absent at one level fall through to
the next, one whole setting at a time. `errors` is a single setting, so a target
config that sets `errors` at all supplies the entire build-system error map and
`default_config`'s `errors` is not consulted for that target. That map is then
merged over the sub-config and top-level maps, so it only has to name the error
kinds it changes.
...
In this example, `MyLib` and `MyPkg` share the `lenient` config and set `root`
to `/src/Project`, so their `srcs` paths are absolutized relative to that
directory (e.g. `Foo.py` becomes `/src/Project/Foo.py`). They report
`missing-import` as a warning, and ignore errors in generated code
from `repo-wide`. The test target names no config, so it is checked with
`repo-wide` alone.
