...
Controls whether Pyrefly interprets jaxtyping annotations such as
`Float[Tensor, "batch channels"]` as tensor shapes. When disabled, Pyrefly uses
the ordinary types exposed by jaxtyping's stubs without enforcing the shape
strings. This enables an experimental subset of jaxtyping's static semantics;
see the [compatibility reference](./tensor-shapes-reference.mdx#jaxtyping-compatibility)
for supported syntax and current limitations.
...
that can apply to a given file when any of their `matches` glob patterns match.
...
- a `matches` key containing either one [Filesystem Glob](#filesystem-globbing) string or a
  non-empty list of glob strings. A list applies the SubConfig when any entry matches.
...
SubConfigs where none of the `matches` entries match the given file, then takes the first non-null
