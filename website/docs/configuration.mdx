...
- Flag equivalent: `--python-platform` (single value only; the list form is config-file only)
...
provide a list such as `python-platform = ["linux", "win32"]` in a
configuration file to type check branches that may run on any of those
platforms.
