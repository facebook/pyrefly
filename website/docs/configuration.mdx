...
### `infer-pytest-fixture-types`

Controls whether unannotated pytest fixture parameters get their type from the
fixture definition with the same name.

When set to `false` (the default), Pyrefly recognizes pytest-injected fixture
parameters and treats them as `Any`. This avoids noisy missing-annotation errors
in test functions while keeping fixture navigation support enabled.

When set to `true`, Pyrefly infers the parameter type from the fixture
definition's return type. Fixtures implemented with `yield` use the yielded type.
